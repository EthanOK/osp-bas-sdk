...
class Bundle {
    ...
    static async newBundle(): Promise<Bundle> {
        ...
        const bundleFile = await fs.promises.mkdtemp(path.join(tempDir, 'tempBundle'));
    ...
    async appendObject(name: string, reader: ReadableStream, options?: AppendObjectOptions): Promise<ObjectMetaType> {
        if (this.finalized) {
        ...
        const written = await new Promise<number>((resolve, reject) => {
            ...
            const readStream = async () => {
                const { done, value } = await readerDefault.read();
                if (done) {
                    resolve(totalWritten);
                    return;
                }
                totalWritten += value.length;
                this.writeFile!.write(value);
                await readStream();
            };
